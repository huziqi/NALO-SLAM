...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        ...
        return torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2)) * 5#10
...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq):
        ...
        # u = self.u.repeat(plane_eq.size(0), plane_eq.size(2) * int(self.upratio), plane_eq.size(3)).cpu()
        u = (u - (self.upratio - 1) * 0.5) / self.upratio
        
        v = self.v.repeat(plane_eq.size(0), plane_eq.size(2), plane_eq.size(3) * int(self.upratio)).cuda()
        # v = self.v.repeat(plane_eq.size(0), plane_eq.size(2), plane_eq.size(3) * int(self.upratio)).cpu()
