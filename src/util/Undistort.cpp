...
ImageAndExposure* Undistort::undistort_mask(ImageAndExposure* image, MaskImg* maskimg, MinimalImageB3* cimage)
{
	//check mask size
	//first check width?= original image width
	//float *mask_crop = new float[wOrg * hOrg];
	// if (maskimg->w != wOrg || maskimg->h > hOrg)
	// {
	// 	printf("Mask image has wrong width with original image! this may segfault. \n");
	// 	return 0;
	// }
	// else
	// {
	// 	//first complete the mask to the size of the orginal image
	// 	for (int col = 0; col < wOrg; col++)
	// 	{
	// 		for (int row = 0; row < hOrg-maskimg->h;row++)
	// 		{
	// 			mask_crop[col + row * wOrg] = 0;
	// 		}
	// 		for (int row = hOrg - maskimg->h, rrow=0; row < hOrg;row++, rrow++)
	// 		{
	// 			mask_crop[col + row * wOrg] = maskimg->data[col + rrow * wOrg];
	// 		}
	// 	}
	// }	
	//crop mask to the same size with image
	...
	// {
	// 	image->mask[ii] = factor*maskimg->data[ii];
	// 	// printf("%f, ", image->mask[ii]);
	// }

	// float* result_mask=new float[w*h];

	// 原尺寸mask
	IOWrap::resizeMask(image->mask, maskimg->data,wOrg,hOrg,w,h);


	// colored images
	IOWrap::resizeColor(image->img_bgr, cimage->data, wOrg, hOrg, w, h);
