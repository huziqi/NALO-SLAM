...
{
public:
	ImageFolderReader(std::string path, std::string calibFile, std::string gammaFile, std::string vignetteFile, std::string mask_path, std::string color_path)
	{
		this->path = path;
		...
		// load file names into vectors using file path
		if(isZipped)
		{
#if HAS_ZIPLIB
			int ziperror=0;
		...
		// load mask names into vectors
		getdir(mask_path, masks);

		// load color images
		getdir(color_path, colors);
...
private:
	...
	ImageAndExposure* getImage_internal(int id, int unused)
	{
		...
		MinimalImageB3 *cimg = IOWrap::readImageRGB_8U(colors[id]);

		MaskImg* maskimg;
		maskimg = IOWrap::readMask_8U(masks[id]);
		// printf("masking w:%u, masking h:%u\n", maskimg->w, maskimg->h);
		ImageAndExposure* ret3 = undistort->undistort_mask(ret2, maskimg, cimg);
	...
	inline void loadTimestamps()
	{
		...
		// std::string timesFile = "/home/hzq/dataset/my_data/s6_livox/times.txt";
