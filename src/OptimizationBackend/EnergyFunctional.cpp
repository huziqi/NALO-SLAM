...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, bool MT)
{
	...
	else
	{
		...
		for(EFFrame* f : frames)
			for(EFPoint* p : f->points)
				accSSE_top_A->addPoint<0>(p,this);//优化的局部信息计算，计算两两帧之间的hessian矩阵
...
//	std::cout << "EigPre:: " << eigenvaluesPre.transpose() << "\n";
	...
	std::vector<VecX> ns;
	ns.insert(ns.end(), lastNullspaces_pose.begin(), lastNullspaces_pose.end());//位姿的零空间
	ns.insert(ns.end(), lastNullspaces_scale.begin(), lastNullspaces_scale.end());//尺度的零空间
...
void EnergyFunctional::solveSystemF(int iteration, double lambda, CalibHessian* HCalib)
{
	...
	if(setting_solverMode & SOLVER_SVD)
	{
		...
		x = SVecI.asDiagonal() * svd.matrixV() * Ub;// 首先求解得到了状态增量x
