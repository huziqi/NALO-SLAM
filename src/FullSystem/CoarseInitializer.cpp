...
bool CoarseInitializer::trackFrame(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	...
	//[ ***step 5*** ] 迭代求解
		...
		while(true)
		{
			...
			doStep(lvl, lambda, inc);//初始化中更新深度
			...
			if(accept)
			{
				...
				if(resNew[1] == alphaK*numPoints[lvl])//? 这是啥   答：应该是位移足够大，才开始优化IR
					snapped = true;
				H = H_new;
				b = b_new;
				Hsc = Hsc_new;
	...
	//[ ***step 6*** ] 优化后赋值位姿, 从底层计算上层点的深度，将逆深度由底层向顶层传播逆深度作为下次优化初值
...
void CoarseInitializer::setFirst(	CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	...
	float densities[] = {0.03,0.05,0.15,0.5,1};//不同层取的点的密度
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		int npts;
		if(lvl == 0)
			npts = sel.makeMaps(firstFrame, statusMap,densities[lvl]*w[0]*h[0],1,false,2);//选择像素点
		else
		...
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
			...
			if((lvl!=0 && statusMapB[x+y*wl]) || (lvl==0 && statusMap[x+y*wl] != 0))
			{
				...
				pl[nl].u = x+0.1;
				pl[nl].v = y+0.1;
				pl[nl].idepth = 1;//整个系统首次对刚选出来的像素点深度赋值
				pl[nl].iR = 1;
				pl[nl].isGood=true;
