...
public:
	...
	float planeOptimize(CalibHessian calib, std::vector<FrameHessian*> fhs);
	float SWGrayOptimize(CalibHessian calib, std::vector<FrameHessian *> fhs);
	float SWGrayOptimize_J(std::vector<FrameHessian *> fhs);
	float SWGrayOptimizeG2o(CalibHessian calib, std::vector<FrameHessian *> fhs);

	bool setglobalplane(std::vector<FrameHessian *> fhs);
	bool setinitgroundheight(std::vector<FrameHessian *> fhs);
	void resetGlobalPlane(std::vector<FrameHessian *> fhs);

	void printResult(std::string file);
	void saveCloudfile(std::string file);
	...
	double gplane[4];
	double backup_gplane[4];

	float acc_r;

	void setGammaFunction(float* BInv);
	void setOriginalCalib(const VecXf &originalCalib, int originalW, int originalH);
	float getlocalgh() { return lgh; };
...
private:
	...
	DenseMapping *densemapper; // 半稠密建图
	cpu_tsdf::TSDFVolumeOctree::Ptr tsdf; // used for tsdf
