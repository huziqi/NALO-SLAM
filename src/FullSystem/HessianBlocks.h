...
struct FrameHessian
{
	...
	float* mask;						// lidar mask, used to select immuature points
	Vec3b *img_bgr;						// colored image
	...
	// 平面相关
	float *last_ground;   // 上一次的平面参数
	Vec4f groundP;		   // 方向向量+距离，4维
	bool haveground=false; // 这一帧是否有可用地面

	bool scaleFixed = false;

	bool havelocalplane = false;
	std::vector<LocalPlane> lplane;
	...
	std::vector<MapPoint*> mapPoints; 				// 局部稠密建图的点云
