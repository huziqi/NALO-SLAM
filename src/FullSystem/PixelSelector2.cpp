...
void PixelSelector::makeHists(const FrameHessian* const fh)
{
	...
	for(int y=0;y<h32;y++)//使用3x3大小的窗口对阈值做平滑
		for(int x=0;x<w32;x++)
		{
			float sum=0,num=0;
			if(x>0)
...
int PixelSelector::makeMaps_lidar(
		const FrameHessian* const fh,
		float* map_out, float density, int recursionsLeft, bool plot, float thFactor)
{
	float numHave=0;
...

	{
		...
		float* map_test=new float[wG[0]*hG[0]];
		memcpy(map_test, map_out, sizeof(float) * wG[0] * hG[0]);
		Eigen::Vector3i m = this->FusedWithMask(fh, map_out);
		// sub-select!
		numHave = m[0]+m[1]+m[2];
	// 	quotia = numWant / numHave;
...
Eigen::Vector3i PixelSelector::FusedWithMask(const FrameHessian* const fh,
		float* map_out)
{
	int wh = wG[0] * hG[0];
	int n1=0, n2=0, n4=0;
	...
	// ############# normalize the mask ######################
	float* normMask=new float[wh];
	memcpy(normMask,fh->mask,sizeof(float)*wh);
		

	// make hist of mask
	int *mhist = new int[256];
	memset(mhist, 0, sizeof(int) * 256);
	for (int ii = 0; ii < wh; ii++)
	{
		if(fh->mask[ii]!=0)
		{
			mhist[0]++;
			int g = (int)fh->mask[ii];
			mhist[g]++;
		}
	}

	// find Quantile
	int th_ = mhist[0]*0.5+0.5f;
	int quantile = 255;
	int max_mas = 0;
	for (int i = 0; i < 256; i++)
	{
	...
	// show the hist
	// MinimalImageB3 hist_(256, 1000);

	// for(int i=0;i<256000;i++)
	// {
	// 	hist_.at(i) = Vec3b(255,255,255);
	// }

	// for(int y=0; y<256;y++)
	// {
	// 	hist_.plotHist(mhist[y],y,Vec3b(0,0,255));
	// }
	// IOWrap::displayImage("hist", &hist_);




	// ##################### add some selected points based on mask ##################
	float rand_sel=0.5;
	std::srand(3141592);
	int rand_n = 0;
	for (int ii = 0; ii < wh; ii++)
	{
		rand_sel = rand() % 1000 / (float)(1000.0);
		if (map_out[ii] == 1)
		{
			n1++;
			// 20% chance to change point status from 1 to 2 based on mask val
			if(rand_sel>0.5&&fh->mask[ii]<quantile/3)
			{
				map_out[ii] = 2;
				n1--;
				n2++;
		...
		else if(map_out[ii]==2)
		{
			n2++;
			//60% chance to change point status from 2 to 1 based on mask val
			if(rand_sel<0.6&&fh->mask[ii]>quantile+(max_mas-quantile)/2) 
			{
				n2--;
				n1++;
				map_out[ii] = 1;
		...
		{
			//30% chance to change point status from 0 to 1 based on mask val
			if(rand_sel<0.01&&fh->mask[ii]>quantile)
			{
				n1++;
				map_out[ii] = 1;
			}
	...
	//printf("mask quantile: %u\n", quantile);

	MinimalImageB3 img(wG[0], hG[0]);
	for(int i=0;i<wh;i++)
	{
		float c = fh->dI[i][0]*0.7;
		if(c>255) c=255;
	...
	for(int y=4; y<hG[0]-4;y++)
		for(int x=4;x<wG[0]-4;x++)
		{
			int i=x+y*wG[0];
			if(map_out[i] == 1)
				img.setPixelCirc(x,y,Vec3b(0,255,0));
	...
	IOWrap::displayImage("selector of Mask", &img);


	return Eigen::Vector3i(n1,n2,n4);
}
